...
- يعمل: كل ما في القسم 6 + REC + **طبقة التنقّل `nav_tools`** (Hand/Zoom/Rotate/Navigator/Space) + **أداة القلم بمستوى احترافي** (عُقد ناعمة صحيحة، إغلاق مرئي، إدراج/حذف عقدة على مسار قائم، تحويل نوع العقدة، قفل 45°، مقابض عند المرور) + **انعكاس ⇋⇵** (المفرد/المجموعات/النص) + **طبقة دقّة** (مؤشّر X/Y حيّ + طول/زاوية + إدخال رقمي للعقدة) + **أداة Curve ◠** (تحويل مستقيم إلى منحنى بالسحب).
